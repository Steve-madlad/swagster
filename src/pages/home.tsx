...
export default function Home() {
  ...
  return (
    <div className="col-full-center gap-3">
      ...
      <div className={`absolute size-screen bg-black/30 ${panelOpen ? "flex-center" : "hidden"}`} onClick={() => setPanelOpen(false)}>
        ...
        <p className="text-3xl font-medium">Available APIs</p>
        <div className="mt-5">
          {
            registry.apis.map(api => {
              return <button onClick={() => navigate(`/api/docs/${api.id}`)} className="border border-black! bg-white! text-black text-start hover:bg-primary! hover:text-white hover:shadow-md hover:border-primary! duration-300 group">
